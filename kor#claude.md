...
1. **남아있는 계획 검토**: `tasks/todo.md`에 체크 안된 항목을 분석후 앞으로 해야할 작업과의 우선순위를 생각하세요.
2. **우선 계획**: `tasks/todo.md`에 체크 가능한 항목으로 계획을 작성하세요.
3. **계획 검증**: 구현을 시작하기 전에 확인을 받으세요.
4. **진행 상황 추적**: 진행하면서 완료된 항목을 표시하세요.
5. **변경 사항 설명**: 각 단계에서 높은 수준의 요약을 제공하세요.
6. **결과 문서화**: `tasks/todo.md`에 검토 섹션을 추가하세요.
7. **교훈 기록**: 수정 사항이 있으면 `tasks/lessons.md`를 업데이트하세요.
